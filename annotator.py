...
import itertools
def extract_categories(match):
    labels = match['category_labels']
    if labels == None: return []
    allcats = set(itertools.chain.from_iterable(labels))
    return sorted(list(allcats))

from collections import defaultdict
def match_to_menutext(match):
    e = defaultdict(lambda: None)
    e.update(match)
    cats = extract_categories(e)
    t = ["\"%s\" (%s)" % (e['name'], ", ".join(cats)),
         "at (%(latitude).06f, %(longitude).06f): %(address)s, %(locality)s" % e]
    return "\n".join(t)
